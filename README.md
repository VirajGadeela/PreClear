...
- **21 requirement rules across all 4 payers**, each carrying document title,
  ...
  are eviCore's, and UnitedHealthcare and Aetna publish their own.
...
Requirement coverage now fills 11 of the 12 (payer x procedure) combinations.
The one gap is **Aetna for head CT, and it is a real absence**: Aetna's Clinical
Policy Bulletin index contains no bulletin setting medical-necessity criteria
for brain or head CT, so there is nothing to cite. An Aetna member ordering a
head CT still gets real prices and a real cost comparison, and the app says on
the results screen that no requirement is recorded rather than quietly showing
one route fewer.
